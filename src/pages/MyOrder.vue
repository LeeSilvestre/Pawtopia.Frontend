        ...
        <div v-if="filterBills.length > 0" class="my-order-cards">
            <div v-for="b in filterBills.slice().reverse()" class="card" :key="b.bill_id">
                ...
                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">
                    ...
                    <div class="w-100 text-center py-1 px-2"><span>Order Date and Time:</span> {{ b.bill_when }}</div>
                </div>
                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">

                    <div class="w-100 text-center py-1 px-2"><span>Total:</span> ₱{{ b.bill_total }}</div>
                    <div class="w-100 text-center py-1 px-2"><span>Name Used:</span>{{ " " + b.bill_address }}
                    </div>
                ...
                <div class="card-body">
                    <div class="steps d-flex flex-wrap flex-sm-nowrap justify-content-between">
                        ...
                        <div class="step" :class="b.bill_status >= 4 ? 'completed' : ''">
                            <div class="step-icon-wrap">
                                <div class="step-icon"><i class="fa-solid fas fa-truck"></i></div>
                            </div>
                            <h4 class="step-title">Now Delivering</h4>
                        </div>
                ...
                <div v-if="b.bill_status === 5" class="receive-button">
                    <button @click="receiveOrder(b.bill_id)">Receive</button>
                </div>
            </div>
        </div>
...
export default {
    ...
    data() {
        return {
            avaiableStatus: ["cancel", "pending", "confirmed", "preparing", "now serving", "delivered", "recieved"],
            allBills: [],
    ...
    methods: {
        ...
        async receiveOrder(billId) {
        // Update the order status on the server
        await axios.put(`/billstatus/${billId}`);

        // Find the index of the received order in the array
        const index = this.allBills.findIndex((b) => b.bill_id === billId);

        // Remove the received order from the array
        if (index !== -1) {
