        ...
        <div class="register-form-container">
            <form id="registerForm" @submit="handleSubmit" novalidate autocomplete="off">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="uName" placeholder="Your full name" id="uName" class="form-control" v-model="registerObj.name" />
                    <p class="error-mess" v-if="errorObj.nameErr.length > 0">{{ errorObj.nameErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uEmail">Enter your email:</label>
                    <input type="email" name="uEmail" placeholder="Must be a valid email" id="uEmail" class="form-control" v-model="registerObj.email" />
                    <p class="error-mess" v-if="errorObj.emailErr.length > 0">{{ errorObj.emailErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uPass">Enter your password:</label>
                    <input type="password" name="uPass" placeholder="Must be at least 8 characters with 1 special character" id="uPass" class="form-control" v-model="registerObj.pass" />
                ...
                <div class="form-group">
                    ...
                    <input type="password" name="uPassConfirm" placeholder="Must match your previously entered password" id="uPassConfirm" class="form-control" v-model="registerObj.confirm" />
                ...
                <div class="form-group">
                    ...
                    <input type="tel" name="uPhone" placeholder="Must be a valid Philippine mobile number" id="uPhone" class="form-control" v-model="registerObj.phone" />
                    <p class="error-mess" v-if="errorObj.phoneErr.length > 0">{{ errorObj.phoneErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uAdd">Enter your address:</label>
                    <input type="text" name="uAdd" placeholder="Input your address" id="uAdd" class="form-control" v-model="registerObj.address" />
                    <p class="error-mess" v-if="errorObj.addressErr.length > 0">{{ errorObj.addressErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uBirth">Enter a valid birthdate:</label>
                    <input type="date" name="uBirth" id="uBirth" class="form-control" @click="availableTime()" v-model="registerObj.birth" />
                ...
                <div class="form-group">
                    ...
                    <div class="form-group">
                        <input type="radio" name="gender" value="male" id="genderMale" v-model="registerObj.gender" /><span>Male</span>
                        <input type="radio" name="gender" value="female" id="genderFemale" v-model="registerObj.gender" /><span>Female</span>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <p>have an account? <router-link @click="scrollToTop()" to="/login">login</router-link></p>
                </div>
            </form>
        </div>
    </div>
...
export default {
    ...
    data() {
        return {
            registerObj: { name: "", email: "", pass: "", confirm: "", phone: "", birth: "", gender: "", address: "" },
            errorObj: { nameErr: [], emailErr: [], passErr: [], addressErr: [], confirmErr: [], phoneErr: [], birthErr: [], genderErr: [] },
            matchUser: undefined,
        }
    },
        ...
        async getMatchUser(email) {
            try {
                ...
                if (error.response.status === 404) {
                    this.matchUser = null; // User not found
                } else {
        ...
        checkForm: function () {
            ...
            if (!this.registerObj.pass) {
                ...
                if (this.registerObj.pass.length < 8) {
                    this.errorObj.passErr.push('Password must be more than or equal to 8 characters');
                }
            }
            ...
            if (!this.registerObj.birth) {
                ...
                if (dateInput === "Invalid Date") {
                    ...
                    if (dateInput.getTime() > eighteenYearsAgo.getTime()) {
                        this.errorObj.birthErr.push("You must be at least 18 years old");
                    }

                    if (dateInput.getTime() < dateMin.getTime() || dateInput.getTime() > dateMax.getTime()) {
                        this.errorObj.birthErr.push("Available birthday range is from the past 150 years to now");
                    }
                }
            }
