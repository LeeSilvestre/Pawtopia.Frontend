        ...
        <div class="checkout-form-container">
            <form id="checkoutForm" @submit="handleSubmit" novalidate autocomplete="off">
                <div class="checkout-heading">
                    ...
                    <h3 v-if="user">{{ user.user_name }}<span>₱{{ calculateSummaryPrice()[3] }}</span></h3>
                </div>

                <div class="form-group details-group">
                    <h4>Order Details</h4>
                    <div class="form-group">
                        <input type="text" name="coPhone" id="coPhone" placeholder="Enter contact number" class="form-control"
                            v-model="checkoutObj.phone" :readonly="!editMode"/>
                        <p class="error-mess" v-if="errorObj.phoneErr.length > 0">{{ errorObj.phoneErr[0] }}</p>
                    </div>

                    <div class="form-group">
                        <input type="text" name="coAddress" id="coAddress" placeholder="Enter your address"
                            class="form-control" v-model="checkoutObj.address" :readonly="!editMode"/>
                ...
                <div class="form-group details-group">
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            ...
                            <input type="radio" name="payment" value="card" id="paymentCard"
                                v-model="checkoutObj.paymentMethod" /><span><img src="../assets/images/gcash-logo.png" alt="GCash Logo" class="gcash-logo" /></span>
                        </div>
                    ...
                    <div v-if="checkoutObj.paymentMethod == 'card'">
                        <div class="form-group">
                            <input type="text" name="coCardNum" placeholder="Enter your gcash number" id="coCardNum"
                                class="form-control" v-model="cardObj.number" size="11" maxlength="11" />
                            <p class="error-mess" v-if="errorObj.numErr.length > 0">{{ errorObj.numErr[0] }}</p>
                        </div>

                        <div class="form-group">
                            <input v-upcase type="text" name="coCardName" placeholder="Enter your gcash account name "
    ...
    methods: {
        ...
        calculateSummaryPrice: function () {
            ...
            while (i < this.itemQuantity.length) {
                subtotal += parseInt(this.filterFoods[i].food_price) * this.itemQuantity[i];
                discount += parseInt(this.filterFoods[i].food_discount) * this.itemQuantity[i];
                i += 1;
            }
        ...
        checkForm: function () {
            ...
            if (!this.checkoutObj.address) {
                this.errorObj.addressErr.push('Entering name is required');
            }
            ...
            if (!this.checkoutObj.paymentMethod) {
                ...
                if (!this.cardObj.number) {
                    this.errorObj.numErr.push('Entering GCash number is required');
                } else {
                    // Adjusted validation for GCash
                    // Assuming GCash numbers start with '09' and have 11 digits
                    if (!this.cardObj.number.startsWith('09')) {
                        this.errorObj.numErr.push('GCash numbers must start with 09');
                    }

                    if (this.cardObj.number.length !== 11) {
                        this.errorObj.numErr.push('GCash numbers must have exactly 11 digits');
                    }

                    if (!/^[0-9]{11}$/.test(this.cardObj.number)) {
                        this.errorObj.numErr.push('GCash numbers can only contain numbers');
                    }
                }
...
<style scoped>
.checkout-container .checkout-form-container form .form-group.details-group button {
...
.checkout-container .checkout-form-container form .form-group.details-group button:hover {
