<template>
        ...
        <div class="row">
            <div class="col-sm-4 col-12 filter-box">
                <div class="row search-box">
                    <input type="text" class="search-input" v-model="foodObj.name" placeholder="Search Pet Product" />
                </div>
                ...
                <div class="row filter-heading">
                    <h1>Filter By Pet Type</h1>
                </div>

                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            <input type="button" name="rType" id="mType" value="dogs" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="mType" class="d-flex justify-content-between">dogs<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rType" id="vType" value="cats" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="vType" class="d-flex justify-content-between">cats<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rType" id="vsype" value="rodents" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="vsype" class="d-flex justify-content-between">rodents<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rType" id="aType" value="birds" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="aType" class="d-flex justify-content-between">birds<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rType" id="kType" value="fishes" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="kType" class="d-flex justify-content-between">fishes<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rType" id="pType" value="others" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="pType" class="d-flex justify-content-between">others<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        ...
                        <li>
                            <input type="button" name="rPrice" id="ltPrice" value="99" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="ltPrice" class="d-flex justify-content-between">{{ "less than" }} ₱100 <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="rtfPrice" value="100,299" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rtfPrice" class="d-flex justify-content-between">₱100 - ₱299 <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="rftPrice" value="300, 499" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rftPrice" class="d-flex justify-content-between">₱300 - ₱499 <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="rttPrice" value="500,999" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rttPrice" class="d-flex justify-content-between">₱500 - ₱999 <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>
                        ...
                        <li>
                            <input type="button" name="rPrice" id="mtPrice" value="1000" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="mtPrice" class="d-flex justify-content-between">{{ "Above" }} ₱1000 <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>
                    ...
                    <div v-for="(f, index) in currentPageItems" :key="index">
                        <div class="box">
                            ...
                            <div class="content">
                                ...
                                <div class="desc">
                                    <p>{{ f.food_qty }} Stocks Left</p>
                                </div>
                                <button class="btn btn-delete" @click="confirmDelete(index)">Remove</button>
                            </div>
                        </div>
                    </div>
    ...
    <div v-if="isDeleteModalOpen" class="custom-delete-modal">
    <div class="custom-modal-inner">
        <h2>Are you sure you want to remove "{{ selectedItem.food_name }}"?</h2>
        <div class="buttons">
            <button class="btn btn-danger" @click="deleteConfirmed">Yes, remove</button>
...
export default {
    ...
    methods: {
        ...
        evaluatePrice: function (food, priceRange) {
            ...
            if (priceRange == "100, 299") {
                if (100 <= cal && cal <= 299) {
                    return food;
                }
            }
            else if (priceRange == "300, 499") {
                if (300 <= cal && cal <= 499) {
                    return food;
                }
            }
        ...
        confirmDelete(index) {
            ...
            const itemId = parseInt(this.currentPageItems[index].food_id);

            // Find the selected item for confirmation
            this.selectedItem = this.filterFoods.find(item => item.food_id === itemId);

            this.isDeleteModalOpen = true;
        },

        deleteConfirmed() {
            this.deleteMenu(); // Perform the actual deletion
            this.isDeleteModalOpen = false; // Close the confirmation modal
        },
        ...
        async deleteMenu() {
            ...
            try {
                ...
                // Update the underlying data property (allFoods) and trigger the computed property to update
                this.$store.commit('deleteFood', this.selectedItem.food_id);
