<template>
    <div class="admin-container">
      <div class="d-flex justify-content-between">
        ...
        <button class="btn manage-menu-btn" @click="goToManageMenu" to="/admin/managemenu">Manage Shop</button>
        <button class="btn manage-menu-btn" @click="goToDelivering" to="/admin/delivering">On Delivery</button>
        <button class="btn manage-menu-btn" @click="goToCancelled" to="/admin/cancelled">Cancelled Orders</button>
        <button class="btn manage-menu-btn" @click="goToHistory" to="/admin/history">History</button>
        <button class="btn" @click="handleLogout()" to="/admin">Logout</button>
      </div>
      <div class="heading">
            <span>Pending Orders</span>
            <div>
            <input type="text" v-model="searchQuery" @input="searchOrders" placeholder="Search by Order ID">
            </div>
      </div>
            ...
            <table class="table colored-header datatable project-list">
                ...
                <tbody>
                    <tr v-for="(order) in filterBills.slice().reverse()" :key="order.bill_id">
                        ...
                        <td>
                    <button v-if="canUpdateStatus(order) && order.bill_status === 1" class="action-btn" @click="nextStatusBtn(order.bill_id)">
                    Confirm
                    </button>
                    <button v-if="canUpdateStatus(order) && order.bill_status === 1" class="cancel-btn" @click="cancelBtn(order.bill_id)">
                    Cancel
                    </button>
                    </td>
                    </tr>
...
export default {
    ...
    data() {
        return {
            searchQuery: '',
            avaiableStatus: ["cancelled", "pending", "confirmed", "packing", "now delivering", "delivered"],
            allBills: [],
            ...
            //filteredFoodsData: [],
            //...bill,
            //orderDetails: this.getOrderDetails(bill.bill_id),
        }
    },
    ...
    computed: {
        ...
        filterBills: function () {
            return this.allBills.filter((b) => b.bill_status !== 5 && b.bill_status < 2 && b.bill_status != 0);
        },
    ...
    methods: {
        ...
        searchOrders() {
            ...
            this.filterBills = this.allBills.filter(order => {
                return order.bill_id.toLowerCase().includes(this.searchQuery.toLowerCase());
            });
        },
        ...
        goToDelivering() {
            console.log("Delivering:", this.admin);
            this.$router.push("/admin/delivering");
        },
        goToCancelled() {
            console.log("Cancelled:", this.admin);
            this.$router.push("/admin/cancelled");
        },
        goToManageMenu() {
